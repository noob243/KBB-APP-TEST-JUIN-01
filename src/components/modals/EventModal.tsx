...
const EventModal: FC<EventModalProps> = ({ isOpen, onClose, onSave, avocats = [] }) => {
    ...
    // Calculate sum of sources de financements (Recettes)
    const recettesTotalCalculated = fundingSources
        .filter(s => s.checked)
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        onSave({ 
          ...
          fraisParticipation: formData.fraisParticipation ? Number(formData.fraisParticipation) : undefined,
          autresRecettes: formData.autresRecettes ? Number(formData.autresRecettes) : undefined,
          recettesTotal: recettesTotalCalculated,
            ...
            <div className="bg-white rounded-2xl shadow-2xl p-6 md:p-8 max-w-4xl w-full max-h-[92vh] overflow-y-auto custom-scrollbar animate-fadeIn">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                <div className="flex justify-between items-center mb-1.5">
                                    <label className="block text-2xs font-bold text-gray-650 uppercase tracking-wide">Sources de financement</label>
                                    <span className="text-[10px] font-black text-emerald-700 bg-emerald-50 border border-emerald-150 px-2 py-0.5 rounded-md">
                                        Recettes (Sources) : {recettesTotalCalculated.toLocaleString('fr-FR')} €
                                    </span>
                                </div>
                                <div className="space-y-1.5 border border-gray-200 rounded-xl p-3 bg-white max-h-[170px] overflow-y-auto custom-scrollbar shadow-inner">
                                    {fundingSources.map((source, index) => (
                                        <div key={source.id} className={`flex items-center justify-between p-1.5 rounded-lg border transition ${source.checked ? 'bg-indigo-50/20 border-indigo-200' : 'bg-white border-gray-200'}`}>
                                            <label className="flex items-center space-x-2 cursor-pointer select-none">
                                                ...
                                                />
                                                <span className={`text-2xs ${source.checked ? 'text-[#15447c] font-black' : 'text-gray-500 font-semibold'}`}>{source.label}</span>
                                            </label>
                                            {source.checked && (
                                                <div className="flex items-center space-x-1">
                                                    <input
                                                        ...
                                                        className="w-20 px-1.5 py-0.5 bg-white border border-gray-300 rounded text-2xs font-extrabold text-gray-800 focus:ring-1 focus:ring-indigo-500 outline-none"
                                                    />
                                                    <span className="text-3xs font-extrabold text-indigo-600">€</span>
                                                </div>
                                            )}
                                        </div>
                                    ))}
                            ...
                            {/* Additional specific money inputs */}
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    <label className="block text-2xs font-bold text-gray-650 uppercase tracking-wide mb-1">Frais de participation</label>
                                    <input 
                                        type="number" 
                                        name="fraisParticipation" 
                                        placeholder="Montant total ou unitaire (ex: 150 €)" 
                                        value={formData.fraisParticipation} 
                                        onChange={handleChange} 
                                        className="w-full text-xs p-2.5 border border-gray-300 rounded-xl focus:border-indigo-500 outline-none" 
                                    />
                                </div>
                                <div>
                                    <label className="block text-2xs font-bold text-gray-650 uppercase tracking-wide mb-1">Autres recettes</label>
                                    <input 
                                        type="number" 
                                        name="autresRecettes" 
                                        placeholder="Subventions, ventes (ex: 500 €)" 
                                        value={formData.autresRecettes} 
                                        onChange={handleChange} 
                                        className="w-full text-xs p-2.5 border border-gray-300 rounded-xl focus:border-indigo-500 outline-none" 
                                    />
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-3 mt-1">
                                <div>
                                    <label className="block text-2xs font-bold text-gray-650 uppercase tracking-wide mb-1">Budget prévisionnel</label>
                                    <input type="number" name="budgetPrevisionnel" placeholder="Coût global prévisionnel €" value={formData.budgetPrevisionnel} onChange={handleChange} className="w-full text-xs p-2.5 border border-gray-300 rounded-xl outline-none focus:border-indigo-500" />
                                </div>
                                <div>
                                    <label className="block text-2xs font-bold text-gray-650 uppercase tracking-wide mb-1">Budget réalisé</label>
                                    <input type="number" name="budgetRealise" placeholder="Coût global réel €" value={formData.budgetRealise} onChange={handleChange} className="w-full text-xs p-2.5 border border-gray-300 rounded-xl outline-none focus:border-indigo-500" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="md:col-span-2 space-y-3 bg-indigo-50/20 p-4 rounded-xl border border-indigo-100/40 relative">
                             ...
                             <div>
                                 ...
                                 <input 
                                     type="text" 
                                     placeholder="Stagiaires, assistants, paralégaux (séparés par des virgules)..." 
                    ...
                    <div className="mt-8 flex justify-end space-x-3 pt-3 border-t border-gray-100">
                        ...
                        <button type="submit" className="bg-indigo-650 text-white font-bold py-2.5 px-6 rounded-xl hover:bg-indigo-700 transition shadow-2xs text-xs">Créer l'Événement</button>
                    </div>
                </form>
            </div>
        </div>

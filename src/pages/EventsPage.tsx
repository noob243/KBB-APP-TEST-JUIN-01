                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex justify-center items-center p-4">
                    <div className="bg-white rounded-2xl shadow-2xl p-6 md:p-8 max-w-xl w-full animate-fadeIn flex flex-col max-h-[90vh]">
                        ...
                        <div className="space-y-4 overflow-y-auto pr-1 flex-1 custom-scrollbar pb-4">
                            ...
                            {/* Finances and budget details */}
                            <div className="p-3.5 bg-slate-50 border border-slate-150 rounded-xl space-y-2.5">
                                <div className="flex justify-between items-center pb-2 border-b border-gray-200">
                                    <span className="text-[10px] font-black text-[#15447c] uppercase tracking-wider block">Recettes & Financement</span>
                                    <span className="text-xs font-black text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded border border-emerald-150">
                                        Recettes (Sources de financement) : {selectedEvent.recettesTotal ? selectedEvent.recettesTotal.toLocaleString('fr-FR') : '0'} €
                                    </span>
                                </div>
                                <div className="grid grid-cols-2 gap-2 text-[10px]">
                                    {selectedEvent.fraisParticipation !== undefined && (
                                        <div className="bg-white border p-2 rounded-lg shadow-3xs">
                                            <span className="text-gray-400 font-bold block">Frais de participation</span>
                                            <span className="text-xs font-black text-gray-800 mt-1 block">{selectedEvent.fraisParticipation.toLocaleString('fr-FR')} €</span>
                                        </div>
                                    )}
                                    {selectedEvent.autresRecettes !== undefined && (
                                        <div className="bg-white border p-2 rounded-lg shadow-3xs">
                                            <span className="text-gray-400 font-bold block">Autres recettes</span>
                                            <span className="text-xs font-black text-gray-800 mt-1 block">{selectedEvent.autresRecettes.toLocaleString('fr-FR')} €</span>
                                        </div>
                                    )}
                                </div>
                                ...
                                {selectedEvent.financements && selectedEvent.financements.length > 0 && (
                                    <div className="pt-1.5 space-y-1">
                                        <span className="text-[9px] font-bold text-gray-450 uppercase tracking-wider block">Détails des Sources :</span>
                                        <div className="grid grid-cols-3 gap-1.5 text-[10px]">
                                            {selectedEvent.financements.map((f, idx) => (
                                                <div key={idx} className="bg-white border border-slate-150 p-2 rounded-md shadow-3xs flex flex-col justify-between">
